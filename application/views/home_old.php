

<!-- start: vertNavBar section -->
<?php include('common/header.php'); ?>
<!-- end: vertNavBar section -->


<main class="content is-userBar-expanded"><div class="row row--b">
    <div class="cell well tci">
        <div class="mbm mbl--m pts tac">
            <h1 class="mbf tci">MASON SPEAKERS</h1>
            <p class="oph tci tsi tsl">Connecting the University with the Community.</p>
        </div>
        <div class="g mbxl--m">
            <div class="g-b g-b--m--1of2">
                <div id="fixVideoBox" class="card card--a card--xs">

                    <iframe width="529" height="315" src="<?php echo $prefs[0]->video_url; ?>" frameborder="0" allowfullscreen></iframe>

                </div>
            </div>
            <div class="g-b g-b--m--1of2">
                <p class="mtm tsxl tfl">
                    <?php echo $prefs[0]->center_content; ?>
                </p>
                <div class="mbl tac tal--m">
                    <a href="<?php echo base_url(); ?>index.php/home/goto_speakerListing" class="btn btn--wide bc-cadet-dark mbs mbf--l">Find a Lecture</a>

                </div>

            </div>
        </div>
    </div>
</div>
    <div class="row">
        <div class="cell well well--l">
            <h2 id="keyNote" class="mbm tac">
                POPULAR SPEAKERS

            </h2>
            <div class="g">
                <div class="g-b g-b--m--1of2 g-b--xl--1of4">
                    <div class="card card--a pathSelect">
                        <div style="float:left;">
                            <strong style="font-size: x-large;">1<sup>st</sup></strong>
                        </div>
                        <div class="badge mbm mhc">
                            <img class="badge-img" height="110" src="<?php echo base_url(); ?>assets/images/jeff_offutt.gif" width="110">
                        </div>
                        <h3 class="pathSelect-title"><span class="pathSelect-title-link pathSelect-title-link--ruby">Jeff Offutt</span></h3>
                        <p class="tsm js-pathDescription">Dr. Jeff Offutt is Professor of Software Engineering at George Mason University.</p>
                        <a href="/paths/ruby" class="btn btn--ruby card-item">View Jeff's Talks</a>
                    </div>
                </div>
                <div class="g-b g-b--m--1of2 g-b--xl--1of4">
                    <div class="card card--a pathSelect">
                        <div style="float:left;">
                            <strong style="font-size: x-large;">2<sup>nd</sup></strong>
                        </div>
                        <div class="badge mbm mhc">
                            <img class="badge-img" height="110" src="<?php echo base_url(); ?>assets/images/Don_Boileau.gif" width="110">
                        </div>
                        <h3 class="pathSelect-title"><span class="pathSelect-title-link pathSelect-title-link--javascript">Don Boileau</span></h3>
                        <p class="tsm js-pathDescription">Dr. Don Boileau is a Professor of Communication at George Mason University. </p>
                        <a href="/paths/javascript" class="btn btn--javascript card-item">View Don's Talks</a>
                    </div>
                </div>
                <div class="g-b g-b--m--1of2 g-b--xl--1of4">
                    <div class="card card--a pathSelect">
                        <div style="float:left;">
                            <strong style="font-size: x-large;">3<sup>rd</sup></strong>
                        </div>
                        <div class="badge mbm mhc">
                            <img class="badge-img" height="110" src="<?php echo base_url(); ?>assets/images/Karen_Bune.gif" width="110">
                        </div>
                        <h3 class="pathSelect-title"><span class="pathSelect-title-link pathSelect-title-link--html-css">Karen Bune</span></h3>
                        <p class="tsm js-pathDescription">Ms. Karen Bune is an adjunct professor at George Mason University where she teaches victimology in the criminal justice department.</p>
                        <a href="/paths/html-css" class="btn btn--html-css card-item">View Karen's Talks</a>
                    </div>
                </div>
                <div class="g-b g-b--m--1of2 g-b--xl--1of4">
                    <div class="card card--a pathSelect">
                        <div style="float:left;">
                            <strong style="font-size: x-large;">4<sup>th</sup></strong>
                        </div>
                        <div class="badge mbm mhc">
                            <img class="badge-img" height="110" src="<?php echo base_url(); ?>assets/images/Maurice_McTigue.gif" width="110">
                        </div>
                        <h3 class="pathSelect-title"><span class="pathSelect-title-link pathSelect-title-link--ios">Maurice McTigue</span></h3>
                        <p class="tsm js-pathDescription">Mr. Maurice McTigue joined the George Mason University Mercatus Center as a Distinguished Visiting Scholar after a career as a New Zealand member of Parliament, Cabinet Minister and Ambassador.</p>
                        <a href="/paths/ios" class="btn btn--ios card-item">View Maurice's Talks</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

<!-- start: vertNavBar section -->
<?php include('common/footer.php'); ?>
<!-- end: vertNavBar section -->

